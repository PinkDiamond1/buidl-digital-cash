...
GET_BLOCKS_CHUNK = 10
HALVENING_INTERVAL = 60*24      # daily (assuming 1 minite blocks)
...
class Node:
    ...
    def validate_block(self, block, validate_txns=False):
        ...
        if validate_txns:
            # Check block timestamps cannot be too far in the future
            ...
            # Validate coinbase separately + fee
    ...
    def get_next_bits(self, block_id, log=False):
        ...
        # Log ssome information (period, target, duration, bits)
        if log:
            logger.info(
                "(dificulty adjustment) "
...
def mine_genesis_block(node, public_key):
    ...
    unmined_block = Block(txns=[coinbase], prev_id=None, nonce=0,
            bits=INITIAL_DIFFICULTY_BITS, timestamp=1546489674.6561358)
    mined_block = mine_block(unmined_block)
